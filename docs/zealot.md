...
1. 应用 module 自动依赖 `update-zealot`，启动约数秒后检查更新并弹窗  
2. Android Studio Gradle 面板 → `uploadApk` → `uploadApkOf{Module}{Variant}` 一键 **clean → assemble → 上传**  

**混淆：** `update-zealot` 已通过 `consumer-rules.pro` 对 `**.BuildConfig` 做 keep（反射读取 `zealot_endpoint` / `zealot_channel_key`）。宿主开启 `minifyEnabled true` 时无需再手写；若自行接入且未依赖本库 consumer rules，请自行 keep：
...
本仓库根工程若包含 `:update-zealot`，脚本默认走 `project(':update-zealot')`（无需等 jitpack tag）。外部工程走 `com.github.skyNet2017.AppUpdate:update-zealot:5.0.1`（需打对应 tag）。
...
```groovy
api 'com.github.skyNet2017.AppUpdate:update-zealot:5.0.1'
// 并自行写入 BuildConfig.zealot_endpoint / zealot_channel_key
```

## 发版给其他 App 用

1. 合并本仓库改动后打 git tag（如 `5.0.1`）并 push  
